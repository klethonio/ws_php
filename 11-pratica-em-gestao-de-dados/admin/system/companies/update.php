...
<div class="content form_create">
    ...
    <article>
        ...
        if (!empty($data['sendPostForm'])) {
            $data['company_status'] = ($data['sendPostForm'] == 'Atualizar' ? '0' : '1');
            unset($data['sendPostForm']);
            $data['company_capa'] = ($_FILES['company_capa']['tmp_name'] ? $_FILES['company_capa'] : 'null');

            require('_models/AdminCompany.class.php');

            $upCompany = new AdminCompany;
            $upCompany->exeUpdate($companyId, $data);

            if (!$upCompany->getResult()) {
                WSMessage($upCompany->getError()[0], $upCompany->getError()[1]);
            } else {
                $_SESSION['success'] = "A empresa <b>{$data['company_title']}</b> foi atualizada no sistema.";
                header('Location: panel.php?view=companies/update&id=' . $companyId);
                exit;
            }
        } else {
            $read = new Read;
            $read->exeRead("app_companies", "WHERE company_id = :id_int", ['id_int' => $companyId]);

            if (!$read->getResult()) {
                $_SESSION['alert'] = 'Você tentou editar uma emrpesa que não existe no sistema.';
                ...
                <label class="label_small">
                    ...
                    <select class="j_loadstate" name="company_state">
                        ...
                        foreach ($read->getResult() as $state):
                            ...
                            if (isset($data['company_state']) && $data['company_state'] == $state_id): echo 'selected';
                            endif;
