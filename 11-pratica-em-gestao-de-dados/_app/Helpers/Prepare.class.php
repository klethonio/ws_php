...
class Prepare
{
     ...
     * @param int $width
     * @param int $height
     * @return string = '<img src="$path" alt="$description" title="$description"/>'
     */
    public static function getImage(string $path, string $description, $width = 0, $height = 0): string
    {
        $width = (int) $width;
        $height = (int) $height;
        if(is_file($path)){
            $home = defined('HOME') ? constant('HOME') : '.';
            return "<img src=\"{$home}/tim.php?src={$home}/{$path}&w={$width}&h={$height}\" alt=\"{$description}\" title=\"{$description}\"/>";
        }
    }

    /**
     * Returns multi-level encoded string
     * @param string $string
     * @return string
     */
    public static function encStr($string) {
