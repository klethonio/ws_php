...
class Pager
{
     ...
     */
    public function __construct(string $appends = null, string $first = null, string $last = null, int $maxLinks = 5, array $ippArray = [10, 25, 50, 100])
    {
        ...
        if (in_array($this->limit, $this->ippArray)) {
            $this->urlQuery = $this->parseUrlQuery(['ipp' => $this->limit, 'page' => '']);
        } else {
            $this->urlQuery = $this->parseUrlQuery(['page' => '']);
        }
    }
     ...
     */
    public function range($page, $limit = null): void
    {;
        $this->page = (int) $page ? $page : 1;
        $this->limit = $this->limit ? $this->limit : (((int) $limit) ? $limit : ((int) $this->ippArray[0]));
        $this->offset = ($this->page * $this->limit) - $this->limit;

        $this->ippArray = array_unique(array_merge($this->ippArray, [$this->limit]));
    }
    ...
    {
        header("Location: {$this->url}{$this->urlQuery}{$this->numPages}");
        exit;
    }
     ...
     * @param array $params
     */
    public function exePagination(string $table, string $condition = null, array $params = null): void
    {
        $this->table = $table;
     ...
     * @return string
     */
    public function renderPagination(): null|string
    {
     ...
     * @return string
     */
    public function renderSelectIpp(): string
    {
        ...
        $urlQuery = $this->parseUrlQuery(['ipp' => "'+this[this.selectedIndex].value+'", 'page' => $this->page]);
        return "<select class=\"pagination-ipp\" onchange=\"window.location='{$this->url}{$urlQuery}';return false\">$items</select>\n";
    }
    ...
    private function getSyntax(): void
    {
        if ($this->rowCount > $this->limit) {
            ...
            if ($this->range[0] > 1) {
                $this->pagination .= "<li><a title=\"{$this->first}\" href=\"{$this->url}{$this->urlQuery}1\">{$this->first}</a></li>";
            }
            if ($this->range[0] > 2) {
            ...
            if ($this->endRange < $this->numPages) {
                $this->pagination .= "<li><a title=\"{$this->last}\" href=\"{$this->url}{$this->urlQuery}{$this->numPages}\">{$this->last}</a></li>";
            }
            $this->pagination .= "</ul>";
        } else {
    ...
    private function calcLinksRange(): void
    {
    ...
    private function getRange(): void
    {
        ...
        for ($iPag = $this->startRange; $iPag <= $endRand; $iPag++) {
            if (in_array($iPag, $this->range)) {
                $this->pagination .= $iPag == $this->page ? "<li class=\"current-page\">{$this->page}</li>" : "<li><a title=\"Página {$iPag}\" href=\"{$this->url}{$this->urlQuery}{$iPag}\">{$iPag}</a></li>";
            }
        }
    }

    private function parseFirstLast(): void
    {
        $this->first = $this->first ? $this->first : '1';
        $this->last = $this->last ? $this->first : (string) $this->numPages;
    }

    private function parseUrlQuery($appendsArr): string
    {
        parse_str(filter_input(INPUT_SERVER, 'QUERY_STRING'), $query);
        $queryArr = array_merge($query, $appendsArr);
